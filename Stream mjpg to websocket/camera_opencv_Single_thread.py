...
class Camera(BaseCamera):
    ...
    def stitch(images, ratio=0.75, reproj_thresh=4.0):
        ...
        if Camera.saved_homo_matrix is None:
            ...
            Camera.bwidth=bwidth+bx
            Camera.bheight=bheight
            Camera.by=by
    ...
    def frames():
        ...
        left_video = cv2.VideoCapture(Camera.left_video_in_path)
        right_video = cv2.VideoCapture(Camera.right_video_in_path)
        ...
        for _ in tqdm.tqdm(np.arange(n_frames)):
            ...
            if ok:
                ...
                yield cv2.imencode('.jpg', stitched_frame)[1].tobytes()
